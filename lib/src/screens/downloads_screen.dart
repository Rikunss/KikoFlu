...
class _DownloadTaskListState extends State<_DownloadTaskList> {
  ...
  List<DownloadTask> _filteredTasks(List<DownloadTask> all) {
    switch (widget.filter) {
      case _DownloadFilter.active:
        return all.where((t) => t.status == DownloadStatus.downloading || t.status == DownloadStatus.converting || t.status == DownloadStatus.paused || t.status == DownloadStatus.pending).toList();
      case _DownloadFilter.failed:
        return all.where((t) => t.status == DownloadStatus.failed).toList();
  ...
  Widget _buildEmptyState(ColorScheme cs, BuildContext context) {
    return Center(
      child: AnimatedSwitcher(
        ...
        child: widget.filter == _DownloadFilter.failed
            ? Column(
                key: const ValueKey('no_failed'),
                mainAxisAlignment: MainAxisAlignment.center,
                children: [
                  Icon(Icons.check_circle_outline, size: 64, color: cs.primary.withValues(alpha: 0.4)),
                  const SizedBox(height: 16),
                  Text('No failed downloads', style: TextStyle(color: cs.onSurfaceVariant)),
                ],
              )
            : widget.filter == _DownloadFilter.completed
                ? Column(
                    key: const ValueKey('no_completed'),
                    mainAxisAlignment: MainAxisAlignment.center,
                    children: [
                      Icon(Icons.download_for_offline_outlined, size: 64, color: cs.primary.withValues(alpha: 0.4)),
                      const SizedBox(height: 16),
                      Text('No completed downloads', style: TextStyle(color: cs.onSurfaceVariant)),
                    ],
                  )
                : Column(
                    ...
                    children: [
                      ...
                      SizedBox(
                        ...
                        child: Stack(
                          ...
                          children: [
                            Icon(Icons.headphones, size: 48, color: cs.primary.withValues(alpha: 0.15)),
                            Positioned(
                              right: 10,
                              bottom: 0,
                              child: Icon(Icons.downloading, size: 32, color: cs.primary.withValues(alpha: 0.3)),
                            ),
                            Positioned(
                              left: 10,
                              top: 0,
                              child: Icon(Icons.library_music, size: 24, color: cs.primary.withValues(alpha: 0.2)),
                            ),
                          ],
                        ),
                      ),
                      const SizedBox(height: 20),
                      Text('No downloads yet', style: Theme.of(context).textTheme.titleMedium?.copyWith(color: cs.onSurfaceVariant)),
  ...
  String _nothingHereMessage() {
    switch (widget.filter) {
      case _DownloadFilter.active:
        return 'All downloads are complete.\nBrowse and download your favorite works.';
      case _DownloadFilter.all:
        return 'Start browsing and download\nyour favorite audio works.';
      default:
        return '';
    }
  }
