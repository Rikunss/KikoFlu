...
public final class FlacEncoder {
	...
	public FlacEncoder(StreamInfo info, int[][] samples, int blockSize, SubframeEncoder.SearchOptions opt, BitOutputStream out) throws IOException {
		this(info, samples, blockSize, opt, out, null);
	}
	
	public FlacEncoder(StreamInfo info, int[][] samples, int blockSize, SubframeEncoder.SearchOptions opt, BitOutputStream out, Consumer<Double> progressCallback) throws IOException {
		...
		for (int i = 0, pos = 0; pos < totalSamples; i++) {
			if (progressCallback != null) {
				progressCallback.accept((double) pos / totalSamples);
			}
